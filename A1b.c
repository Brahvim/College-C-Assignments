...
int main() {
  ...
  puts(
      "Please note that these results are in the same currency as the "
...
char get_user_input_for(double *p_input_number_storage_addr) {
  const int scan_result = scanf("%lf", p_input_number_storage_addr);  // Obtaining user input.
  clear_stdin();

  if (scan_result == EOF) {
    fputs(
        "An error occurred trying to read your input! Please restart "
        "this program...\n",
        stderr);
    return FALSE;
  }
